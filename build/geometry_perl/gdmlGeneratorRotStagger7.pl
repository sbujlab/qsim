...
print def "<union name=\"quartzSol_${opt_T}_$j\">
   ...
   <position name=\"quartzCutPos_${opt_T}_$j\" unit=\"mm\" x=\"",$dx[$j]*(0.5)+$dzRef[$j]*tan($refTopOpeningAngle[$j])-$dxLg[$j]/cos($lgTiltAngle[$j])+$dxLg[$j]*cos($lgTiltAngle[$j])/2-$dzLg[$j]*sin($lgTiltAngle[$j])/2,"\" y=\"0\" z=\"",$dzRef[$j]+$dz[$j]*(0.5)+$dzLg[$j]*cos($lgTiltAngle[$j])/2+$dxLg[$j]*sin($lgTiltAngle[$j])/2+2,"\"/>
