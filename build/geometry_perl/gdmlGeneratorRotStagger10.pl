...
use strict 'vars';
use vars qw($opt_F $opt_T $data $line @fields @index @x @y @z @dx @dy @dz @rx @ry @rz @quartzCutAngle @refTopOpeningAngle @dzRef @dxLg @dyLg @dzLg  @lgTiltAngle @dxPmt @dyPmt @dzPmt @wallThick @wtOverlap @pmtWindowThickness @range  $i $j $k $width);
...
print def "<union name=\"quartzSol_${opt_T}_$j\">
    ...
    <rotation name=\"quartzCutRot_${opt_T}_$j\" unit=\"rad\" x=\"pi/2\" y=\"0\" z=\"pi\"/> 
</union>\n";



print def "<trap name = \"refLogicSol_${opt_T}_$j\" z=\"$dzRef[$j]\" theta=\"",atan((($dzRef[$j]*tan($refTopOpeningAngle[$j])-($dxLg[$j]-$dx[$j])*0.5))/$dzRef[$j]),"\" phi=\"0\" y1=\"",$dy[$j]+2*$width,"\" x1=\"",$dx[$j]+2*$width,"\" x2=\"",$dx[$j]+2*$width,"\" alpha1=\"0\"
y2=\"",$dyLg[$j]+2*$width,"\" x3=\"",$dxLg[$j]+2*$width,"\" x4=\"",$dxLg[$j]+2*$width,"\" alpha2=\"0\" aunit=\"rad\" lunit= \"mm\"/>\n";


print def "<trap name = \"lgLogicSol_${opt_T}_$j\" z=\"$dzLg[$j]\" theta=\"",$lgTiltAngle[$j]*(-1),"\" phi=\"0\" y1=\"",$dyLg[$j]+2*$width,"\" x1=\"",$dxLg[$j]+2*$width,"\" x2=\"",$dxLg[$j]+2*$width,"\" alpha1=\"0\" y2=\"",$dyPmt[$j]+2*$width,"\" x3=\"",$dxPmt[$j]+2*$width,"\" x4=\"",$dxPmt[$j]+2*$width,"\" alpha2=\"0\" aunit=\"rad\" lunit= \"mm\"/>\n";
...
print def "<union name=\"logicSol1_${opt_T}_$j\">
    ...
    <position name=\"quartzCutPos_${opt_T}_$j\" unit=\"mm\" x=\"",(($dzRef[$j]*tan($refTopOpeningAngle[$j])-($dxLg[$j]-$dx[$j])*0.5))*0.5,"\" y=\"0\" z=\"",$dzRef[$j]*(0.5)+$dz[$j]*(0.5),"\"/>
...
print def "<union name=\"quartzLogicSol_${opt_T}_$j\">
   ...
   <position name=\"quartzCutPos_${opt_T}_$j\" unit=\"mm\" x=\"",0.5*$dx[$j]+$dzRef[$j]*tan($refTopOpeningAngle[$j])-0.5*$dxLg[$j]+$dzLg[$j]*0.5*tan($lgTiltAngle[$j]*(-1)),"\" y=\"0\" z=\"",$dzRef[$j]+$dz[$j]*(0.5)+$dzLg[$j]*0.5,"\"/>
    <rotation name=\"quartzCutRot_${opt_T}_$j\" unit=\"rad\" x=\"0\" y=\"",0,"\" z=\"0\"/> 
</union>\n";
...
print def "<trap name = \"lgSol_${opt_T}_$j\" z=\"$dzLg[$j]\" theta=\"",$lgTiltAngle[$j]*(-1),"\" phi=\"0\" y1=\"$dyLg[$j]\" x1=\"$dxLg[$j]\" x2=\"$dxLg[$j]\" alpha1=\"0\" y2=\"$dyPmt[$j]\" x3=\"$dxPmt[$j]\" x4=\"$dxPmt[$j]\" alpha2=\"0\" aunit=\"rad\" lunit= \"mm\"/>\n";
...
print def "<volume name=\"quartzVol_${opt_T}\_$j\">
        ...
        <physvol name=\"ref_${opt_T}_$j\">
			...
			<position name=\"detectorPos_${opt_T}_$j\" unit=\"mm\" x=\"",(($dzRef[$j]*tan($refTopOpeningAngle[$j])-($dxLg[$j]-$dx[$j])*0.5))*0.5,"\" y=\"0\" z=\"",$dz[$j]*(0.5)+$dzRef[$j]*(0.5),"\"/>
			<rotation name=\"detectorRot_${opt_T}_$j\" unit=\"rad\" x=\"",0,"\" y=\"0\" z=\"0\"/>
</physvol> \n
        ...
        <physvol name=\"lg_${opt_T}_$j\">
			...
			<position name=\"detectorPos_${opt_T}_$j\" unit=\"mm\" x=\"",0.5*$dx[$j]+$dzRef[$j]*tan($refTopOpeningAngle[$j])-0.5*$dxLg[$j]+$dzLg[$j]*0.5*tan($lgTiltAngle[$j]*(-1)),"\" y=\"0\" z=\"",$dzRef[$j]+$dz[$j]*(0.5)+$dzLg[$j]*0.5,"\"/>
			<rotation name=\"detectorRot_${opt_T}_$j\" unit=\"rad\" x=\"",0,"\" y=\"",0,"\" z=\"",0,"\"/>
</physvol> \n
