...
qsimMonitorDetector::qsimMonitorDetector( G4String name, G4int detnum ) : G4VSensitiveDetector(name){
    char colname[255];
...
qsimMonitorDetector::~qsimMonitorDetector(){
}

void qsimMonitorDetector::Initialize(G4HCofThisEvent *){
    fHitColl = new qsimMonitorDetectorHitsCollection( SensitiveDetectorName, collectionName[0] );
}

///////////////////////////////////////////////////////////////////////

G4bool qsimMonitorDetector::ProcessHits( G4Step *step, G4TouchableHistory *){
...
//    G4double edep = step->GetTotalEnergyDeposit();
    ...
    qsimMonitorDetectorHit *thishit;
    if( !badhit ){
	thishit = new qsimMonitorDetectorHit(fDetNo, copyID);
...
void qsimMonitorDetector::EndOfEvent(G4HCofThisEvent*HCE) {
