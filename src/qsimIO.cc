...
void qsimIO::InitializeTree(){
    ...
    //
    fTree->Branch("hit.pe",    &fNDetEp,     "hit.pe/I");
    //
    ...
    fTree->Branch("hit.eff",    &fDetHit_Eff,   "hit.eff[hit.n]/D"); 
    //
...
void qsimIO::SetEventData(qsimEvent *ev){
    ...
    fEvPart_kinE = sqrt(ev->fPartMom[0].mag()*ev->fPartMom[0].mag()
	    + ev->fPartType[0]->GetPDGMass()*ev->fPartType[0]->GetPDGMass() )
	-  ev->fPartType[0]->GetPDGMass()/__E_UNIT;
...
//	G4cerr << "WARNING: " << __PRETTY_FUNCTION__ << " line " << __LINE__ << ":  Buffer size exceeded!" << G4endl;
    ...
    fNDetHit++;
    //
    //double qeff = CLHEP::RandFlat::shoot( 0.0, 1.0);
