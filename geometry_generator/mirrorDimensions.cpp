...
cout.precision(3);
	...
	// ------------------- INPUT FREE PARAMETERS ----------------------------
		...
		// Add space to provide airgap for mirrors. This will be removed before .txt is written.
	...
	// ------------------- WRITE PARAMETERS TO TEXT FILE ----------------------------
			...
			// Remove spce that was provided for airgap for mirrors.
		...
		if (myfile.is_open()){
			...
			myfile << "         = new G4PVPlacement(rotTung,G4ThreeVector(-" << qt << "*mm/4,0,quartz_zPos-quartz_z-6*mm/2),tungsten_box_log,\"tungsten\", det_log,false,0); " << endl << endl;
			
			myfile << "         G4VPhysicalVolume* tungsten_box_phys_2" << endl;
			
			myfile << "         = new G4PVPlacement(rotTung,G4ThreeVector(-" << qt << "*mm/4,0,quartz_zPos-3*quartz_z-3*6*mm/2),tungsten_box_log,\"tungsten\", det_log,false,0); " << endl << endl;
			
			myfile << "         G4VPhysicalVolume* tungsten_box_phys_3" << endl;
			
			myfile << "         = new G4PVPlacement(rotTung,G4ThreeVector(-" << qt << "*mm/4,0,quartz_zPos-5*quartz_z-5*6*mm/2),tungsten_box_log,\"tungsten\", det_log,false,0);" << endl << endl;
			
			myfile << "         G4VPhysicalVolume* tungsten_box_phys_4" << endl;
			
			myfile << "         = new G4PVPlacement(rotTung,G4ThreeVector(-" << qt << "*mm/4,0,quartz_zPos-7*quartz_z-7*6*mm/2-0.5*mm),tungsten_box_log,\"tungsten\", det_log,false,0); // REPLACE END" << endl;
			...
			myfile << "        G4ThreeVector zTrans_8_1(" << qt/2 << "*mm/2,0.0*mm,-7*" << qt << "*mm/2-3*6*mm-0.5*mm/2-0.01*mm);" << endl << endl;
		...
		//system("emacs mirrorTranslations.txt &");
	system("perl translateSHMX.pl > qsimDetectorConstructionDifferences.out");
	  
	  char replace;
	 
	  cout << " Replace qsimDetectorConstruction.cc with new values? (y/n): " << endl;
	  cin >> replace;
	  
	  if (replace=='y') {
	    
	    system("cp qsimDetectorConstructionNew.cc ../qsim/qsim-master/src/qsimDetectorConstruction.cc");
